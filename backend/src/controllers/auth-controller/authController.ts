...
import { pubRedis } from "../../redis/pub";
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    //📒📒when you using Redis sessions, we wont need  Creating a JWT token and Sending that token as a cookie.
    //     When we switch to a session-based system, Redis acts as your server-side “state memory”.
    // That means:

    // You no longer need to generate or send a JWT.
    // Because the session itself is the proof that the user is logged in.
    // The browser keeps only the session ID cookie (e.g. connect.sid), and the server looks up the session in Redis.

    // You don’t need to manually set your own cookie, because express-session already does that automatically.📒📒
